...
func TestCreateWithAccessToken(t *testing.T) {
	...
	newAPI, err := CreateWithAccessToken(testVersion, testClientID, forceAPI.oauth.AccessToken, forceAPI.oauth.InstanceURL)
	if err != nil {
		t.Fatalf("Unable to create new force api instance using pre-defined oauth details: %#v", err)
	}
	if err := newAPI.oauth.Validate(); err != nil {
		t.Fatalf("Oauth object is invlaid: %#v", err)
	}
	...
	_, err = newAPI.DescribeSObject(&sobjects.Account{})
	if err != nil {
